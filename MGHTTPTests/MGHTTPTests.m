...
@interface MGHTTPTests : XCTestCase

@end

@implementation MGHTTPTests
...
- (void)testURLRequest{
    ...
    [MGHTTP get:@"https://apple.com" andResult:^(NSData *data, NSURLResponse *response, NSError *error){
...
- (void)testURLRequestWithNilParameters{
    ...
    [MGHTTP get:@"https://apple.com" andParameters:nil andResult:^(NSData *data, NSURLResponse *response, NSError *error){
        XCTAssertNil(error, @"request failed with error %@", error);
        NSInteger statusCode = [(NSHTTPURLResponse *) response statusCode];
        XCTAssertEqual(statusCode, 200, @"status code was not 200; was %ld", statusCode);
        XCTAssert(data, @"received data is nil");
